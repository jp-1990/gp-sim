...
describe('CreateLivery', () => {
  ...
  describe('select livery files section', () => {
    ...
    it('correctly renders the selectLivery section table based on File input, with the error message updated and a remove button. When remove is clicked, the item should no longer be rendered', async () => {
      ...
      expect(
        screen.queryByRole('status', {
          name: `${formLabels.liveryFiles}-error`
        })
      ).not.toBeInTheDocument();
      ...
      expect(
        screen.getByText(
          formStrings.invalidDynamicLiveryFileName.defaultMessage
        )
      ).toBeInTheDocument();
      for (const file of requiredLiveryFileNames) {
        await user.click(
  ...
  describe('public livery checkbox', () => {
  ...
  describe('private garage checkbox', () => {
  ...
  describe('select garage and garage key fields', () => {
